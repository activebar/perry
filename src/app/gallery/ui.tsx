        ...
        {feed.map(it => (
          <div key={it.id} className="rounded-2xl border border-zinc-200 overflow-hidden">
            ...
            <div className="p-3 space-y-2">
              {!selectMode ? (
                <>
                  <div className="flex items-center justify-between gap-1 whitespace-nowrap" dir="rtl">
                    <Button variant="ghost" onClick={() => shareItem(it)} type="button" className="min-w-0 rounded-full px-2.5 py-2 text-sm">🔗</Button>
                    <Button variant="ghost" onClick={() => downloadUrl(it.url)} type="button" className="min-w-0 rounded-full px-2.5 py-2 text-sm">{DOWNLOAD_ICON}</Button>
                    <div className="relative">
                      <Button
                        variant="ghost"
                        onClick={() => setEmojiPickerForId(prev => (prev === it.id ? null : it.id))}
                        type="button"
                        className="min-w-0 rounded-full px-2.5 py-2 text-sm"
                      >
                        {(() => {
                      ...
                      {emojiPickerForId === it.id ? (
                        <div className="absolute left-0 bottom-full mb-2 flex items-center gap-1 rounded-full border border-zinc-200 bg-white px-2 py-1 shadow-lg">
                          {EMOJIS.map(emo => (
                            <button
                              key={emo}
                              type="button"
                              onClick={() => toggleReaction(it.id, emo)}
                              className={`rounded-full px-2 py-1 text-lg ${myReactionById[it.id] === emo ? 'bg-black text-white' : 'bg-white'}`}
                              aria-label={`הגב ${emo}`}
                            >
                              {emo}
                            </button>
