            ...
            <div className="mt-3 rounded-2xl bg-white/95 p-3 shadow-lg" dir="rtl">
              <div className="flex flex-wrap items-center justify-center gap-2">
                {EMOJIS.map(emo => {
                  ...
                  return (
                    <Button key={emo} variant={active ? 'primary' : 'ghost'} className="min-w-[54px] rounded-full px-3" onClick={(e) => { e.stopPropagation(); ensureDeviceId(); toggleReaction(lightbox.id, emo) }} type="button">
                      {c ? `${c} ` : ''}{emo}
                    </Button>
                  )
                })}
            ...
            <div className="p-3">
              {selectMode ? (
                ...
                <div className="flex items-center justify-center gap-6 text-2xl">
                  <button type="button" onClick={() => downloadUrl(it.url)} aria-label="הורד" className="leading-none">⬇️</button>
                  <button type="button" onClick={() => setLightbox(it)} aria-label="תגובות" className="leading-none">👍</button>
                  <button type="button" onClick={() => shareItem(it)} aria-label="שתף" className="leading-none">🔗</button>
                </div>
              )}
            </div>
          </div>
