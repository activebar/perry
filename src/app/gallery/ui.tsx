...
function mapItem(x: any): Item {
  return {
    ...
    reaction_counts: x.reaction_counts || emptyCounts(),
    my_reactions: Array.isArray(x.my_reactions) ? x.my_reactions : []
  }
}
...
function secondsLeftFor(item?: Item | null, nowTs = Date.now()) {
  if (!item?.editable_until) return 0
  const ms = new Date(item.editable_until).getTime() - nowTs
...
async function ensureShortLinkForMedia(mediaItemId: string, eventId: string) {
...
export default function GalleryClient({
    ...
    const pull = async () => {
      try {
        ...
        if (!cancelled && Array.isArray(j?.items)) {
          setItems(prev => {
            const prevById = new Map<string, Item>(prev.map(it => [it.id, it] as [string, Item]))
            return j.items.map((x: any) => {
              const next = mapItem(x)
              const old = prevById.get(next.id)
              return old ? { ...next, reaction_counts: old.reaction_counts || emptyCounts(), my_reactions: old.my_reactions || [] } : next
            })
          })
        }
      } catch {
  ...
  const reactionIds = useMemo(() => feed.map(it => it.id).filter(Boolean), [feed])
    ...
    ;(async () => {
      try {
        const res = await fetch('/api/public/media-reactions', {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ ids: reactionIds })
        })
        const j = await res.json().catch(() => ({}))
        if (!res.ok || cancelled || !j?.items) return
        const bundle = j.items as Record<string, { counts?: Record<string, number>; my?: string[] }>
        setItems(prev => prev.map(it => {
          const hit = bundle[it.id]
          return hit ? { ...it, reaction_counts: hit.counts || emptyCounts(), my_reactions: hit.my || [] } : it
        }))
      } catch {
        // ignore
      }
  ...
  async function toggleReaction(mediaItemId: string, emoji: string) {
    try {
      ...
      const j = await res.json().catch(() => ({}))
      if (!res.ok) throw new Error(j?.error || 'שגיאה בריאקשן')
      setItems(prev => prev.map(it => it.id === mediaItemId ? { ...it, reaction_counts: j.counts || emptyCounts(), my_reactions: j.my || [] } : it))
      setLightbox(prev => prev && prev.id === mediaItemId ? { ...prev, reaction_counts: j.counts || emptyCounts(), my_reactions: j.my || [] } : prev)
    } catch (e: any) {
            ...
            <div className="mt-3 rounded-2xl bg-white/95 p-3 shadow-lg" dir="rtl">
              ...
              <div className="mt-3 flex flex-wrap items-center justify-center gap-2">
                {EMOJIS.map(emo => {
                  ...
                  const count = (lightbox.reaction_counts || emptyCounts())[emo] || 0
                  return (
                    <Button
                      key={emo}
            ...
            >
              ...
              {selectMode ? (
                <div className="absolute left-2 top-2">
                  <div className={`h-7 w-7 rounded-full border bg-white/90 flex items-center justify-center text-sm ${selected[it.id] ? 'font-bold' : ''}`} aria-hidden>
                    {selected[it.id] ? '✓' : ''}
                  </div>
                </div>
              ) : null}
              ...
              {!selectMode ? (
                <div className="flex flex-wrap items-center justify-center gap-2" dir="rtl">
                  {EMOJIS.map(emo => {
                    ...
                    const count = (it.reaction_counts || emptyCounts())[emo] || 0
                    return (
                      <Button
                        key={emo}
                        variant={active ? 'primary' : 'ghost'}
