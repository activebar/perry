...
export async function POST(req: Request) {
  ...
  const fallbackTarget =
    kind === 'gl' ? (postId ? `/gallery/p/${postId}` : '/gallery') : postId ? `/blessings/p/${postId}` : '/blessings'

  const code = cleanCode(body.code || (postId ? postId.slice(0, 8) : ''))
  ...
  // (1) code + kind + post_id + target_path
  // (2) code + kind + target_path
  // (3) code + post_id + target_path (legacy)
  // (4) code + target_path (legacy)
  const attempts: any[] = []
  if (postId) attempts.push({ code, kind, post_id: postId, target_path: targetPath })
  attempts.push({ code, kind, target_path: targetPath })
  if (postId) attempts.push({ code, post_id: postId, target_path: targetPath })
