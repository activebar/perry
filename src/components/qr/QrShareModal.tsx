...
export default function QrShareModal({
  ...
  async function onCopy() {
    try {
      ...
      // fallback: select text is overkill; ignore silently
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onMouseDown={onClose}>
      ...
      >
        ...
        <div className="mt-4 grid grid-cols-1 gap-2">
          ...
          <a className="rounded-xl bg-emerald-600 px-4 py-3 text-center text-white" href={waLink} target="_blank" rel="noreferrer">
            {btnWhatsappLabel}
          </a>
        </div>

        <div className="mt-3 rounded-xl bg-zinc-50 p-3 text-xs break-all text-zinc-700">
          {url}
        </div>
      </div>
    </div>
