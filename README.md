...
## Blessings approvals and limits

שדה קיים
approval_lock_after_days

משמעות
מספר ימים מיום האירוע start_at שלאחריהם כל ברכה עוברת לאישור מנהל

שדה חדש
max_blessing_lines

משמעות
מקסימום שורות לברכה לפני שליחה לאישור מנהל
ברירת מחדל 50

התנהגות
ברכה שעוברת את המקסימום נשמרת ומסומנת כממתינה לאישור
המשתמש יכול לערוך ולשלוח שוב


## Soft moderation for blessings

כל ברכה נבדקת גם בסינון תוכן אוטומטי.
אם הברכה סומנה כבעייתית, היא לא נחסמת למשתמש.
היא נשמרת ומועברת לסטטוס ממתין לאישור מנהל, והמשתמש יכול לערוך ולשלוח שוב.

משתני סביבה להפעלה
OPENAI_API_KEY
OPENAI_MODERATION_MODEL אופציונלי ברירת מחדל omni-moderation-latest

שדות שנוספו לטבלת posts
moderation_flagged
moderation_provider
moderation_raw
pending_reason
