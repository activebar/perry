...
export async function PUT(req: NextRequest) {
  ...
  const sb = supabaseServiceRole()
  const { data, error } = await sb.from('media_items').update({ is_approved }).eq('id', id).select('*').single()
  if (error) return jsonError(error.message, 500)
...
export async function DELETE(req: NextRequest) {
  ...
  const sb = supabaseServiceRole()
  const { data: row, error: rerr } = await sb.from('media_items').select('id, storage_path').eq('id', id).single()
